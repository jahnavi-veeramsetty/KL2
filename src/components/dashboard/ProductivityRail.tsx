...
function Panel({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('rounded-2xl border border-white/[0.06] bg-primary/80 p-4', className)}>
      {children}
    </div>
  )
}
...
function WeekActivity() {
  ...
  return (
    <Panel>
      <div className="flex items-baseline justify-between gap-3 mb-3">
        <h3 className="text-sm font-bold text-white tracking-tight">Activity this week</h3>
        <span className="text-[11px] font-semibold text-slate-400 tabular-nums">{activeDays} of 7</span>
      </div>
        ...
        {week.map(day => (
          <span
            ...
            className={cn(
              ...
              day.date === todayISO ? 'bg-accent' : day.count > 0 ? 'bg-accent/30' : 'bg-white/[0.06]'
            )}
  ...
  return (
    <Panel>
      ...
      <div className="grid grid-cols-2 gap-2">
        {cells.map(cell => (
          <div key={cell.label} className="rounded-xl border border-white/[0.07] bg-white/[0.03] px-3 py-2.5">
            <p className={cn('text-base font-bold tracking-tight tabular-nums leading-none', cell.tone)}>
              {cell.value}
            </p>
            <p className="text-[9px] font-bold uppercase tracking-[0.1em] text-slate-500 mt-1.5">
              {cell.label}
            </p>
          </div>
        ))}
      ...
      <div className="mt-3 pt-3 border-t border-white/[0.07] flex items-baseline justify-between gap-3">
        <span className="text-[11px] text-slate-500">Daily challenges</span>
